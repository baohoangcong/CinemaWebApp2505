	...
	<div class="container mt-5">
		<div class="row" style="width: 1500px">
				...
				<c:if test="${not empty listThongTinPhim}">
					<c:forEach var="c" items="${listThongTinPhim}">
						<tr style="border: 1px black solid;">
							...
							<td scope="col"><form
									action="${pageContext.request.contextPath}/admin/phimupdatedangchieu/${c.maPhim}">
			...
			<c:if test="${currentPage > 1}">
				<a href="listPhimDangChieu?page=${currentPage-1}">Previous</a>
			</c:if>
			...
			<c:if test="${currentPage lt totalPages}">
				<a href="listPhimDangChieu?page=${currentPage+1}">Next</a>
			</c:if>
		</div>
			</div>
			<!-- Form list -->
